...
export default function VerifyOtp() {
  ...
  const handleResend = async () => {
    if (!username.trim()) {
      setError('Username is required to resend OTP.');
      return;
    }
    setLoading(true);
    setError('');
    ...
    try {
      ...
      setSuccessMessage(response.data?.message || 'A fresh code has been sent to your email.');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Verification successful, redirect to login page with success feedback
      navigate('/login', {
    ...
    <div className="flex min-h-[80svh] flex-col justify-center py-12 px-6 lg:px-8 bg-slate-50">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-sm border border-slate-200 rounded-xl sm:px-10">
          ...
          {successMessage && (
            <div className="mb-6 p-4 text-sm text-green-755 bg-green-50 border border-green-200 rounded-lg" role="alert">
              <div className="flex">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {/* Username field (editable only if not passed from Signup) */}
            {!passedUsername && (
              <div>
